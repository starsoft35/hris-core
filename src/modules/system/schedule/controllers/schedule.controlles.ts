...
import { entityExistResponse, postSuccessResponse, genericFailureResponse } from 'src/core/helpers/response.helper';
...
export class ScheduleController extends BaseController<Schedule> {
  ...
  async create(
    ...
    try {
      const isIDExist = await this.scheduleService.findOneByUid(createEntityDto.id);
      ...
      if (isIDExist !== undefined) {
        ...
        const createdEntity = await this.scheduleService.create(createEntityDto);
