      ...
      for (let id of dx) {
        for (let orgUnit of ou) {
          for (let period of this.getISOPeriods(pe)) {
            ...
            let query =
              "SELECT * FROM indicator WHERE uid ='" + indicatoruid + "';";
            let indicators = await this.connetion.manager.query(query);
            indicators = [
              {
                expression: 'COUNT(*)',
                filter:
                  'created>=startOfReportingPeriod AND created<=endOfReportingPeriod',
              },
            ];
            ...
            let levelquery = orglevels.map(
              orglevel =>
                ...
                " = '" +
                orgUnit +
                "' AND data.ou=ous.uidlevel" +
                orglevel.level,
            );

            query = `SELECT ${
              indicators[0].expression
            } FROM _resource_table_${formuid} data
        INNER JOIN _organisationunitstructure ous ON((${levelquery.join(
          ') OR (',
        )})) 
        INNER JOIN _periodstructure pe ON(pe.iso='${period}') 
        WHERE ${indicators[0].filter
          .replace('startOfReportingPeriod', 'pe.startdate')
          .replace('endOfReportingPeriod', 'pe.enddate')}`;
            //+ 'INNER JOIN _periodstructure pes ON(' + periodquery.join(' OR ') + ')';
            console.log(query);
            let result = await this.connetion.manager.query(query);
            analytics.rows.push([id, orgUnit, period, result[0].count]);
          }
        }
      }
      return analytics;
